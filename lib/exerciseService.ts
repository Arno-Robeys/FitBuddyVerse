...
async function getExerciseIDs(id: number, session: any): Promise<number[]> {
	try {
		...
		return [
			...new Set(data.exerciseSets?.map((set) => set.exerciseId)),
		] as number[];
	} catch (err) {
		console.log(err);
	}
...
async function getExerciseWithSets(
	id: number,
	session: any
): Promise<TExercise> {
	const res = await fetch(
		`${process.env.EXPO_PUBLIC_URL}/exercises/${id}/sets`,
		{
			headers: {
				authorization: "Bearer " + session?.user.accessToken,
			},
...
async function getExercise(id: number, session: any): Promise<TExercise> {
	const res = await fetch(`${process.env.EXPO_PUBLIC_URL}/exercises/${id}`, {
		headers: {
			authorization: `bearer ${session?.user?.accessToken}`,
		},
	});
...
async function getExercises(session: any): Promise<TExercise[]> {
	const res = await fetch(`${process.env.EXPO_PUBLIC_URL}/exercises`, {
		headers: {
			authorization: "Bearer " + session?.user.accessToken,
		},
	});
...
const createExercise = async (
	{ name, type, equipment }: Omit<TExercise, "id">,
	session: any
) => {
	const res = await fetch(process.env.EXPO_PUBLIC_URL + "/exercises", {
		method: "POST",
