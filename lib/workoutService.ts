...
async function getWorkout(id: number, session: any): Promise<TWorkout> {
    const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/workouts/${id}`, {
        headers: {
...
async function getWorkoutEmbedAll(id: number, session: any): Promise<TWorkout> {
    const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/workouts/${id}?embed=all`, {
        headers: {
            authorization: 'Bearer ' + session?.user.accessToken,
        },
    });
...
const createWorkout = async (
    { name, durationSec, likeCount, volumeKG, profileId }: Omit<TWorkout, 'id' | 'createdAt'>,
    session: any
): Promise<TWorkout> => {
    const res = await fetch(process.env.NEXT_PUBLIC_URL + '/workouts', {
        method: 'POST',
        body: JSON.stringify({
            name,
            durationSec,
